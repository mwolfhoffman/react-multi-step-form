  ...
  return (
    <>
        ...
        <div className="flex justify-between px-4 py-2">
          <div className="text-marineBlue">
            <h3 className="font-medium">
              {formState.billingPlan} (
              {formState.billingCycle === 'yr' ? 'Yearly' : 'Monthly'})
            </h3>
            <a
              className="text-coolGray hover:bg-gray-100 cursor-pointer hover:underline"
      ...
      <div className="flex justify-between p-4">
        <span className=" text-coolGray font-light">
          Total (per {formState.billingCycle === 'yr' ? 'year' : 'month'}):{' '}
        </span>
        <span className="text-marineBlue font-extrabold">
          ${calculateTotal()}/{formState.billingCycle}
        </span>
      </div>
    </>
  )
